      ...
      <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-8">
          ...
          <p class="text-light-paragraph dark:text-dark-paragraph">
            Complete katakana character chart with romaji pronunciation
          </p>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg p-6 shadow">
          <div class="overflow-x-auto">
            <table class="w-full text-center border-collapse">
              ...
              <tbody>
                <tr *ngFor="let row of katakanaChart" class="border-t border-secondary dark:border-slate-700">
...
export class KanaKatakanaComponent implements OnInit {
  loading$ = this.kanaService.loading$;

  katakanaList: Kana[] = [];
  katakanaChart: { label: string; kana: (Kana | null)[] }[] = [];
  ...
  private loadKatakana(): void {
    this.kanaService.getAllKana('katakana').subscribe({
      next: (kana) => {
        this.katakanaList = kana;
        this.katakanaChart = this.organizeKanaChart(kana);
      },
      error: (err) => console.error('Error loading katakana:', err)
    });
  }
